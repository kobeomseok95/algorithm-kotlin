...
class `11_2_Repeat` {
    fun maxArea(height: IntArray): Int {
        ...
        while (left < right) {
            val minHeight = minOf(height[right], height[left])
            answer = maxOf(answer, getArea(width = right - left, height = minHeight))
    ...
    private fun getArea(width: Int, height: Int): Int {
        return width * height
    }
}

fun main() {
    val solution = `11_2_Repeat`()
